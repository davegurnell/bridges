...
object Rename {
  ...
  def rename[A](value: A, from: String, to: String)(implicit rename: Rename[A]): A =
    rename.apply(value, from, to)

  def instance[A](func: (A, String, String) => A): Rename[A] =
    new Rename[A] {
